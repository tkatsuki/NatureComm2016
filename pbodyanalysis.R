...
pbodynum <- function(dir, mol, molthresh=0.1, pbodythresh=0.05, cadch, pbodych, molch){
  ...
  lapply(filelist, function(x) dir.create(paste(dir, strsplit(x, ".lsm")[[1]], sep="/")))
  ...
  for(i in 1:length(filelist)){
    images <- readLsm(paste(dir, "/", filename[[i]], ".lsm", sep=""))
    h <- dim(images)[1]
    w <- dim(images)[2]
    cad <- images[,,cadch,1]
    img <- images[,,molch,1]
    pbody <- images[,,pbodych,1]
    ...
    # Creating nanos2 or dazl mask
    molint <- computeFeatures.basic(cmask11, img)[1]
    imgblr1 <- gblur(img, 3, 5)
    imgblr2 <- gblur(img, 5, 9)
    imgdn <- imgblr1 - imgblr2
    molthresh <- 0.48*molint
    nmask <- thresh(imgdn, 100, 100, molthresh)
    nmask3 <- bwlabel(nmask)
    nsize <- computeFeatures.shape(nmask3)
    nid <- which(nsize[,'s.area'] < 4)
    nmask4 <- rmObjects(nmask3, nid)
    nmask5 <- nmask4 > 0
    nmask6 <- Image(array(c(img+nmask5, img*(1-nmask5), img+nmask5), dim=c(h, w, 3)))
    colorMode(nmask6) <- "Color"
    
    cellst <- stackObjects(cmask10, cmask10) > 0
    edgest <- dilate(cellst, makeBrush(3, "diamond")) - cellst
    molst <- stackObjects(cmask10, nmask5)
    pbodyst <- stackObjects(cmask10, pmask5)
    cellstint <- apply(stackObjects(cmask10, img), 3, mean)
    mergest <- rgbImage(edgest+(molst>0),edgest+(pbodyst>0),edgest+(molst>0))
    pbodymolst <- stackObjects(cmask10, pmask5*nmask5)
    molstnum <-  apply(bwlabel(molst), 3, max)
    pbodystnum <-  apply(bwlabel(pbodyst), 3, max)
    pbodymolstnum <-  apply(bwlabel(pbodymolst), 3, max)
    cells[[i]] <- data.frame(file=rep(paste(dir, "/", filename[[i]], sep="")), 
                             pbodystnum, molstnum, pbodymolstnum, cellstint)
    
    # Mask pbody with germ cell mask
    germpbody <- pmask5 * cmask11
    germmol <- nmask5 * cmask11
    germmolpbody <- germpbody * germmol
    germmolpbodyintav <- computeFeatures.basic(germmolpbody, img)
    germpbodyintav <- computeFeatures.basic(germpbody, img)
    germintav <- computeFeatures.basic(cmask11, img)
    
    # Save processed images
    writeImage(rgbImage(pbody, pbody, pbody), paste(dir, "/", filename[[i]], "/pbody.png", sep=""))
    writeImage(pbodybg, paste(dir, "/", filename[[i]], "/pbodybg.png", sep=""))
    writeImage(pbodydn, paste(dir, "/", filename[[i]], "/pbodydn.png", sep=""))
    writeImage(pmask, paste(dir, "/", filename[[i]], "/pbodymask.png", sep=""))
    writeImage(pmask4, paste(dir, "/", filename[[i]], "/pbodymaskdn.png", sep=""))
    writeImage(pmask6, paste(dir, "/", filename[[i]], "/pbodyaddmask.tif", sep=""))
    writeImage(rgbImage(cad, cad, cad), paste(dir, "/", filename[[i]], "/cad.png", sep=""))
    writeImage(cadblur, paste(dir, "/", filename[[i]], "/cadblur.png", sep=""))
    writeImage(cmask3, paste(dir, "/", filename[[i]], "/cadmask3.png", sep=""))
    writeImage(cmask4, paste(dir, "/", filename[[i]], "/cadmask4.png", sep=""))
    writeImage(cmask5, paste(dir, "/", filename[[i]], "/cadmask5.png", sep=""))
    writeImage(normalize(cmask10), paste(dir, "/", filename[[i]], "/cadmask10.png", sep=""))
    writeImage(normalize(cad + (cmask9>0)*0.3), paste(dir, "/", filename[[i]], "/cadmask9m.png", sep=""))
    writeImage(normalize(cad + (cmask10>0)*0.3), paste(dir, "/", filename[[i]], "/cadmask9m2.png", sep=""))
    writeImage(cadc + (cmask12>0)*0.3, paste(dir, "/", filename[[i]], "/cadmask12m.png", sep=""))
    writeImage(rgbImage(img, img, img), paste(dir, "/", filename[[i]], "/", mol, ".png", sep=""))
    writeImage(imgdn, paste(dir, "/", filename[[i]], "/", mol, "dn.png", sep=""))
    writeImage(nmask, paste(dir, "/", filename[[i]], "/", mol, "mask.png", sep=""))
    writeImage(nmask4, paste(dir, "/", filename[[i]], "/", mol, "maskdn.png", sep=""))
    writeImage(nmask6, paste(dir, "/", filename[[i]], "/", mol, "addmask.png", sep=""))
    writeImage(germpbody, paste(dir, "/", filename[[i]], "/", "pbodyfinal", ".png", sep=""))
    writeImage(germmol, paste(dir, "/", filename[[i]], "/", mol, "final", ".png", sep=""))
    writeImage(germmolpbody, paste(dir, "/", filename[[i]], "/", mol, "pbodyoverlap", ".png", sep=""))
    writeImage(molst, paste(dir, "/", filename[[i]], "/", mol, "molst", ".png", sep=""))
    writeImage(mergest, paste(dir, "/", filename[[i]], "/", mol, "mergest", ".png", sep=""))
    
    npbody[[i]] <- max(bwlabel(germpbody))
    nmol[[i]] <- max(bwlabel(germmol))
    nmolpbody[[i]] <- max(bwlabel(germmolpbody))
    npbody_per_germ[[i]] <- max(bwlabel(germpbody))/ngerm[[i]]
    nmol_per_germ[[i]] <- max(bwlabel(germmol))/ngerm[[i]]
    nmolpbody_per_germ[[i]] <- max(bwlabel(germmolpbody))/ngerm[[i]]
    ifelse(is.null(germmolpbodyintav), intmolpbody[[i]] <- NA, intmolpbody[[i]] <- germmolpbodyintav[[1]])
    ifelse(is.null(germpbodyintav), intpbody[[i]] <- NA, intpbody[[i]] <- germpbodyintav[[1]])
    ifelse(is.null(germintav), intgerm[[i]] <- NA, intgerm[[i]] <- germintav[[1]])
